...
| `SHARED/` | Cross-cutting protocol documents every stage depends on — handoff schema, document governance/versioning, process-scaling tiers, hard constraints, reuse/license rules, session continuity, sprint ceremonies, task tracking (`PROJECT_TRACKING.md`), how to start a session against this skill (`SESSION_STARTUP.md`, `STARTUP_PROMPTS.md`), where pre-GO intake output lives (`INTAKE_REPOSITORY.md`), and the mandatory existing-code audit before scoping epics (`EXISTING_CODE_AUDIT_GATE.md`). |
